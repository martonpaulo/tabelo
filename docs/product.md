...
| Source axis selection | A selected row or column in a source view is drawn as the grid's: an unbroken band over the column's cells (divider and continuation lines included) or across the row's cells, the active cell in the focus mark, and one current line for the main selection only | design system §9 | owner, 2026-09-19 |
