...
| Interface motion | Three moves only, each transform and opacity over one of four shared tokens: a floating layer opening and closing, the grid's focus mark travelling between cells, and a row or column an insert command added; nothing else animates, none of them delays a command, and reduced motion stands them all down | design system §2, §7 | owner, 2026-09-19 |
...
| Source position mapping | A codec declares whether its parse maps each row and cell to source offsets, only where the mapping is exact; Markdown, CSV, TSV, and Jira did first, and JSON, Records, and HTML followed with #402; an unparsed draft maps nothing | ADR 0005 | #255, #402 |
| Block source rows | JSON, Records, and HTML map each row from their own parse as a block (an object, a title line through its bullets, a `<tr>`), every line of which names it, and their cells by column; a header spelled as keys has no text and names nothing; they declare no header line of columns (`mapsSourceColumns`), so they show no letters, pin no header, and align nothing | ADR 0005, design system §9 | #402 |
