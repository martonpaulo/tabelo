...
| Inline editing | The Visual Table formats the complete text of each selected textual cell as one history step, adds links and images to one cell through dialogs, and edits text in a restricted rich cell editor built on the browser's editing surface with the document as the only model | ADR 0011 | #306 |
| Inline syntax per format | Markdown, HTML, and Jira spell inline content in one canonical syntax each and never infer it from ordinary text; HTML input is untrusted; CSV, TSV, JSON, and Records disclose the projection instead; only https images load and only web and email links open | ADR 0011 | #306 |
...
| Source column markers | Every source view whose codec maps its header cells shows the grid's column letters above its text, placed over the header line's cells, wrapped or not (on the header's first visual line); presentation only | design system §9 | #368 |
