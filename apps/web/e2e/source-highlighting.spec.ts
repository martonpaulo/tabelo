...
import { outsidePinnedHeader, type TabeloPage } from "./helpers";
...
test("JSON scalar roles use distinct semantic and upright treatments", async ({
	...
	await expect(
		pane.locator(`.cm-line span${outsidePinnedHeader}`).first(),
	).toBeVisible();

	const tokens = await pane
		.locator(`.cm-line span${outsidePinnedHeader}`)
		.evaluateAll((spans) =>
			spans.map((span) => {
